...
namespace FluentValidation.Internal {
		...
		public class ValidationStrategy<T> {
				...
				/// <summary>
				/// Indicates that the specified properties should be validated.
				/// </summary>
				...
				/// <param name="properties">The property names to validate.</param>
				/// <returns></returns>
				public ValidationStrategy<T> FilterProperties(params string[] properties) {
						if (_properties == null) {
								_properties = new List<string>(properties);
						}
						else {
				...
				private IValidatorSelector GetSelector() {
						...
						if (_properties != null || _ruleSets != null || _customSelector != null) {
								...
								selector = selectors.Count == 1 ? selectors[0] : new CompositeValidatorSelector(selectors, _isFilter);
						}
						else {
